...
// Parse the collection key into language and bits
...
// Get timeline years from 3-bit pattern
export function getTimelineYears(bits: string, startPosition: number): number {
  if (startPosition < 0 || startPosition + 2 >= bits.length) {
  ...
  switch (timelineBits) {
    ...
    case '111': return 7;
    case '110': return 10;
    default: return 1; // Default to 1 year for invalid patterns
  }
}

// Generate requirements object from collection key
